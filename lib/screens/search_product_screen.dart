...
class BottomBar extends StatelessWidget with FilterScreenController {
  @override
  Widget build(BuildContext context) {
    void autoGenerateProduct() async {
      FoodData foodData = FoodData();
      Provider.of<Products>(context).clearProduct();
      ...
      List<Product> autoGeneratedProducts = new List();
      while (autoGeneratedProducts.length < 10) {
        //Get a new random food name
        String randomFood = foodNames[_random.nextInt(foodNames.length)];
        DocumentSnapshot documentSnapshot = await foodData.getEntry(randomFood);
        List<dynamic> recipe = documentSnapshot.data['Recipe'];
        for (int i = 0; i < 10; i++) {
          Product product = await foodData.decodeProduct(recipe[i]);
          if (product.name != null)
          ...
          if (product.isHealthy != false) {
            Provider.of<Products>(context).addProduct(product);
            autoGeneratedProducts.add(product);
          }
        }
      }
      Provider.of<Products>(context).updateDisplayProduct('all');
